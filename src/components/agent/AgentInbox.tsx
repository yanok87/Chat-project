...
export function AgentInbox({ threads, selectedThreadId, onSelect }: AgentInboxProps) {
  ...
  return (
    <ul
      ref={listRef}
      className="flex flex-col border-r border-gray-200 bg-gray-50 overflow-y-auto"
      role="listbox"
          ...
          return (
            <li key={thread.id}>
              <button
                ...
                className={`w-full text-left px-4 py-3 border-b border-gray-200 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-500 ${
                  isSelected ? "bg-white border-l-4 border-l-blue-600" : "hover:bg-gray-100"
                }`}
                role="option"
              ...
              >
                ...
                <p className="mt-0.5 text-sm text-gray-900 truncate">{preview}</p>
              </button>
            </li>
          );
        })
